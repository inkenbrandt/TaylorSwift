...
def _add_slope_line(ax, slope, f_range=(0.5, 5.0), label=None, anchor_y=None, **kwargs):
...
def plot_cospectra(
    ...
    panels = [
        (
            ax_wT,
            "ncosp_wT",
            "$n \\cdot Co_{wT} \\,/\\, \\overline{w'T'}$",
            "Sensible heat flux cospectrum",
            "wT",
        ),
        (
            ax_wu,
            "ncosp_wu",
            "$n \\cdot Co_{wu} \\,/\\, \\overline{w'u'}$",
            "Momentum flux cospectrum",
            "wu",
        ),
        (
            ax_wCO2,
            "ncosp_wCO2",
            "$n \\cdot Co_{wCO_2} \\,/\\, \\overline{w'CO_2'}$",
            "CO$_2$ flux cospectrum",
            None,
        ),
        (
            ax_wH2O,
            "ncosp_wH2O",
            "$n \\cdot Co_{wH_2O} \\,/\\, \\overline{w'H_2O'}$",
            "H$_2$O flux cospectrum",
            None,
        ),
    ]
    ...
    for ax, attr, ylabel, title, flux_type in panels:
        ...
        if show_model and flux_type is not None:
            ...
            ax.plot(
                f_model,
                kaimal_cospec_model(f_model, flux_type),
                "k-",
                lw=2.0,
                alpha=0.8,
                label="Kaimal (1972)",
        ...
        if show_slope:
            _add_slope_line(
                ax, -4.0 / 3.0, f_range=(1, 10), label="$f^{-4/3}$", anchor_y=0.1
            )
            ax.legend(fontsize=9, loc="upper right")
...
def plot_spectra(
    ...
    for ax, attr, ylabel, title in panels:
        ...
        _add_slope_line(
            ax, -2.0 / 3.0, f_range=(1, 10), label="$f^{-2/3}$", anchor_y=0.05
        )
        ax.legend(fontsize=9, loc="upper right")
        ax.set_xlim(1e-3, 20)
        ax.set_ylim(1e-4, 5)
...
def plot_ogive(
    ...
    labels = [
        ("ogive_wT", "cov_wT", "$Og_{wT}$ / $\\overline{w'T'}$", "Heat flux ogive"),
        ("ogive_wu", "cov_wu", "$Og_{wu}$ / $\\overline{w'u'}$", "Momentum ogive"),
        (
            "ogive_wCO2",
            "cov_wCO2",
            "$Og_{wCO_2}$ / $\\overline{w'CO_2'}$",
            "CO$_2$ ogive",
        ),
        (
            "ogive_wH2O",
            "cov_wH2O",
            "$Og_{wH_2O}$ / $\\overline{w'H_2O'}$",
            "H$_2$O ogive",
        ),
    ]
